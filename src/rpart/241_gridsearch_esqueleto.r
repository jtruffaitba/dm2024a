...
PARAM$semillas <- c(123457, 123479, 123491, 123493, 123499)
...
##############################################################################
# falta que itere por cp y minbucket
# agregar las iteraciones para esos dos parámetros.
# buscar en biblio los valores posibles para cp
# en la librería rpart

# cp: complejidad minima
cpList = -0.5#seq(-1, 2, by = 0.5)
...
for (vmax_depth in c( 8)) {
  for (vmin_split in c(650)) {
    ...
    for (cp in cpList) {
      ...
      for (minBucket in unique(as.integer(vmin_split*c(1/2)))){
        ...
        print(paste0("Loop:","cp: ", cp, " vmax_dept: ", vmax_depth, " vimn_split: ", vmin_split, " minBucket:", minBucket))
      }
